...
public class ResourceManager {
    ...
    protected static final Map<String, WeakReference<?>> cacheMap = 
    ...
    protected static <T> void addToCache(Class<T> resourceType, String resourceName, T value){
    ...
    public static <T> T loadResource(Class<T> resourceType, String resourceName){
        try {
            ...
            loadingResourcesSet.remove(resourceName);
            addToCache(resourceType, resourceName, val);
        ...
        executor.submit(
                new Callable<Object>(){
                    public Object call() throws ExecutionException {
                        try {
                            loadingResourcesSet.add(resourceName);
                            addToCache(resourceType,
                                       resourceName,
                                       loader.load(resourceName));
                            loadingResourcesSet.remove(resourceName);
                        } catch (IOException ex) {
                            onException(ex);
                        }
