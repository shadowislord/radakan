...
void LinuxKeyboard::capture()
{
	...
	while( XPending(display) > 0 )
	{
		...
		if( KeyPress == event.type )
		{
			...
			/*int bytes = */XLookupString(&event.xkey, 0, 0,&key, 0);

			_injectKeyDown(key, event.xkey.time, buffer[0]);
...
//-------------------------------------------------------------------//
bool LinuxKeyboard::_injectKeyDown( KeySym key, unsigned int time, int text )
{
	...
	if( mBuffered && listener )
		return listener->keyPressed(KeyEvent(this,time,kc,text));
...
bool LinuxKeyboard::_injectKeyUp( KeySym key, unsigned int time )
{
	...
	if( mBuffered && listener )
		return listener->keyReleased(KeyEvent(this, time, kc, 0));
