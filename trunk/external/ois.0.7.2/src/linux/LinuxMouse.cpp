...
void LinuxMouse::capture()
{
	...
	if( mMoved == true )
	{
		...
		mState.relX = XMouseX - oldXMouseX;
		mState.relY = XMouseY - oldXMouseY;
		...
		if( grabMouse )
		{
			...
			if( mState.abX < 0 ) mState.abX = 0;
			else if( mState.abX > mState.width ) mState.abX = mState.width;

			if( mState.abY < 0 ) mState.abY = 0;
			else if( mState.abY > mState.height ) mState.abY = mState.height;
	...
	while( XPending(display) > 0 ) 
	{
		...
		else if( event.type == ButtonPress )
		{
			...
			if( event.xbutton.button < 4 )
			{
				...
				if( mBuffered && listener )
					if( listener->mousePressed( MouseEvent( this, event.xbutton.time, mState ),
		...
		else if( event.type == ButtonRelease )
		{
			if( event.xbutton.button < 4 )
			{
				mState.buttons &= ~mask[event.xbutton.button];
				if( mBuffered && listener )
					if( listener->mouseReleased( MouseEvent( this, event.xbutton.time, mState ),
						(MouseButtonID)(mask[event.xbutton.button] >> 1)) == false )
						return;
			}
