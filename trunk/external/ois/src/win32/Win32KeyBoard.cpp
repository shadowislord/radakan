...
const std::string& Win32Keyboard::getAsString( KeyCode kc )
{
	...
	//Does not work for extended keys, so...
	if( GetKeyNameText(kc << 16,temp,sizeof(temp)) != 0 )
		mGetString = temp;
	else
	{
		//Well, I cannot seem to get some keys to show up..
		//so, just use the KC_***
		std::ostringstream ss;
		ss << "Key_" << kc;
		mGetString = ss.str();
		//if( GetKeyNameText((0x1<<25)|(kc<<16),temp,sizeof(temp)) != 0 )
		//	mGetString = temp;
	}
