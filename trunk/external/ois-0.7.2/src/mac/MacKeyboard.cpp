...
//-------------------------------------------------------------------//
MacKeyboard::MacKeyboard( bool buffered, bool repeat )
{
...
{
    OSStatus status;
    WindowRef window = ( (MacInputManager*)(InputManager::getSingletonPtr()) )->_getWindow();
...
void MacKeyboard::injectEvent( KeyCode kc, unsigned int time, MacEventType type, unsigned int txt )
{
    ...
    if ( mBuffered && listener ) {
        pendingEvents.push_back( MacStackEvent( KeyEvent(this, time, kc, txt), type) );
    }
}
